...
void LeptonThread::run()
{
	...
	while(true) {
		...
		for(int j=0;j<PACKETS_PER_FRAME;j++) {
			...
			ssize_t readResult = read(spi_cs0_fd, result+sizeof(uint8_t)*PACKET_SIZE*j, sizeof(uint8_t)*PACKET_SIZE);
			if (readResult < 0) {
				log_message(3, "Error reading from SPI device");
				continue;
			}
		...
		QRgb color;
		float maxTemp = -273.15f; // 최저 온도로 초기화
		for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				// Lepton 2.x와 3.x의 온도 변환 계수가 다름
				float temperature;
				if (typeLepton == 3) {
					temperature = (valueFrameBuffer - 27315) / 100.0f; // Lepton 3.x
				} else {
					temperature = (valueFrameBuffer - 27315) / 100.0f; // Lepton 2.x
				}
		...
		curl = curl_easy_init();
		if(curl) {
			std::string jsonData = "{\"temperature\":" + std::to_string(maxTemp) +
								   ",\"timestamp\":" + std::to_string(time(NULL)) +
								   ",\"device_id\":\"raspberry_pi\"}";

			curl_easy_setopt(curl, CURLOPT_URL, "http://34.173.146.75:3000/temperature");
			...
			if(res != CURLE_OK) {
				log_message(3, "curl_easy_perform() failed: " + std::string(curl_easy_strerror(res)));
			}
